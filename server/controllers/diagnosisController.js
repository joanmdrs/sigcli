...
import { getAllDiagnosisByAppointment, deleteDiagnosisByID,getOneDiagnosisByID, updateDiagnosisByID } from '../repositories/diagnosisRepository.js';
...
export const updateDiagnosis = async (req, res) => {
    const {diagnosis_id} = req.params;
    const {id,illness, description, patient_cpf, doctor_crm, appointment_id} = req.body;
    ...
    try {
        ...
        const diagnosis = await updateDiagnosisByID(diagnosis_id,diagnosisBody);
        res.status(200).json(diagnosis);
